...
trait Json {
  ...
  implicit val encodeCreatePolicyJson: EncodeJson[CreatePolicy] = EncodeJson { cp =>
    ...
    def path(rule: Rule): (String, argonaut.Json) =
      rule.path := {
        ("policy" :?= rule.policy) ->?:
  ...
  def jsonMount(path: String): DecodeJson[Mount] = DecodeJson { c =>
    ...
    } yield Mount(path, typ, desc, defaultLease, maxLease)
  }
